...
// Resumen del mes con importes reales: cada gasto cuenta en un solo
// sitio. Si su concepto contiene el nombre de un gasto fijo, es fijo;
// si no, es variable y va a su categoría (la elegida al apuntarlo o,
// si no tiene, la que encaje por el texto) o a "Otros".
export function calcularResumenMes(movDelMes, gastosFijos, categorias) {
  const fijos = gastosFijos.map((gf) => ({ concepto: gf.concepto, centimos: 0 }));
  ...
  for (const m of movDelMes) {
    ...
    const iFijo = gastosFijos.findIndex((gf) => concepto.includes(gf.concepto.toLowerCase()));
    if (iFijo !== -1) {
      fijos[iFijo].centimos += gasto;
      continue;
    }
    let iCat = m.categoria_id ? categorias.findIndex((c) => c.id === m.categoria_id) : -1;
    if (iCat === -1) iCat = categorias.findIndex((c) => concepto.includes(palabraClave(c.concepto)));
    if (iCat !== -1) variables[iCat].centimos += gasto;
    else otros += gasto;
  }
  if (otros > 0) variables.push({ concepto: "Otros", centimos: otros });
  ...
  return {
    ...
    fijos: fijos.map((f) => ({ concepto: f.concepto, importe: euros(f.centimos) })),
...
function hojaMovimientos(filas) {
  ...
  for (let c = 0; c < 5; c++) ws[XLSX.utils.encode_cell({ r: 0, c })].s = cabecera;
  filas.forEach((_, i) => {
    for (let c = 0; c < 5; c++) {
      ...
      if (c === 2) estilo.font = { color: { rgb: ROJO } };
      if (c === 3) estilo.font = { color: { rgb: VERDE } };
      celda.s = estilo;
    }
  });
  ws["!cols"] = [{ wch: 12 }, { wch: 30 }, { wch: 13 }, { wch: 13 }, { wch: 13 }];
  return ws;
}

export function construirLibroExcel({ nombreMes, movDelMes, filas, gastosFijos, categorias }) {
  const resumen = calcularResumenMes(movDelMes, gastosFijos, categorias);
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, hojaResumen(nombreMes, resumen), "Resumen");
  XLSX.utils.book_append_sheet(wb, hojaMovimientos(filas), "Movimientos");
  return wb;
}
